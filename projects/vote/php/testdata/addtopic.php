...
function getTopicInfo($title, $desc,$starttime, $user_id, $is_official)
{
    $create_time = time();
...
$topicSql = "INSERT INTO elastos_topic (title,start_time,start_date,end_time,user_id,choice,is_official,create_time,type,status,answer_status,is_valid,start_gmt_time, end_gmt_time) VALUES ";
...
// 从文件中读取数据到PHP变量
$json_string = file_get_contents($options['f']);
...
{
    $addTopicsql = $topicSql . getTopicInfo($value['title'], $value['desc'], $value['Starttime'], $officialId, $options['o']);
